...
import { Protocol, type TLSInfo } from '../tls';
...
describe('validate', () => {
  ...
  it('returns errorMessage for soon to expire certificate', () => {
    expect(
      ...
      protocolNotApproved: false,
      errorMessage:
        'Issues found with certificate - Certificate will expire in less than 40 days'
    });
  });
  ...
  it('returns multiple errorMessages if applicable', () => {
    expect(
      ...
      protocolNotApproved: true,
      errorMessage:
        'Issues found with certificate - Invalid protocol: unknown, Certificate will expire in less than 40 days'
    });
  });
});
